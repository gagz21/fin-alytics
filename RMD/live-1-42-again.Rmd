...
house_prices <- read.csv("data/HousePrices.csv")
# subset the data
...
These live session break-outs will provide practice with `R` variables, arrays, calculations, basic descriptive statistics, and plotting. 
...
Let's see if the `house_prices` object has been loaded into this workspace by typing `ls()` below. 
...
Look up `quantile()` using the `?` help operator.
...
Why didn't we do this before?! Why the `NA`s? `4` what? Some cleaning to do. But let's handle that in another session. Boxplots? Very interesting indeed!
...
We made it through our first set of break-outs. 
...
More R next week: we will focus on pivots, lookups, merges, and functions.